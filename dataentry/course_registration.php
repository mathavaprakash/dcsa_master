...
<div id="page-wrapper">
		...
		<section id="main" class="container">
         ...
         <h3>Select the Staff</h3>




 
<form name='course_registration.php' method='POST' action="#">
                                    <div class="row uniform 50%">
                                   <div class="12u 12u(mobilep)">
<select id="batch" name="Staff_Key"> 
<?php
include_once "../assets/db/dbdcsa.php";
/*mysql_connect("localhost","root","") or die(mysql_error());
mysql_select_db("dcsa") or die("Cannot connect to database");*/
$query="select * from mas_staff";
$res=mysql_query($query);
while ($row=mysql_fetch_array($res))
{?>
    <option value="<?php echo $row['Staff_Key'];?>"><?php echo $row['First_Name']; ?></option>
<?php
}
?>
</select>
...
{       
  $batch_id=mysql_real_escape_string($_POST['Staff_Key']);
   include_once "../assets/db/dbdcsa.php";
   $batch=encryptor('encrypt',$batch_id);
   print '<script>window.location.assign("course_entry.php?Staff_Key='.$batch.'");</script>';
}
?>
</html>