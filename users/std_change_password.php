		...
		<link rel="stylesheet" href="../assets/css/main.css" />
            ...
            jQuery(function(){
                ...
                <?php            
                    ...
                    //$encrp=$_SESSION['encr'];  
                    $regno=  encryptor('decrypt',$_SESSION['encregno']);  
                    $_SESSION['encregno']=encryptor('encrypt',$regno);
            ...
            if($exist>0)
            {
                while($row=  mysql_fetch_assoc($query))
                {
					...
					if($regno==$table_userid )
                    {
						if($oldpwd==$table_oldpwd)
						{
							if($newpwd==$conpwd)
							{
								 mysql_query("update mas_students SET `Password`='$newpwd' WHERE `Student_Key` = '$regno'");
                ...
                if($temp==0)
                {
                    print '<script>alert("your old Password is wrong. ")</script>';
                     header("location:std_change_password.php");
                }
            }       
        }
?>
