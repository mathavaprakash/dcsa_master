...
<form action="attendancesearch.php" method="post">
...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
  if(isset($_POST['submit2']))
  {
	...
	$sql2=mysql_query("select * from stud_attend1");
  echo '	 
  <div class="row">
  <div class="12u">
  <section class="box">
	...
	if($t=mysql_fetch_array($sql2))
	{
		...
		do{
			...
			{ 
				 ...
				 if($tdmonth==date("M",strtotime($datearray1[$i+1])))
				 {
				   if($i==count($valarray)-2)
				   {
					 $tcount++;
					 $tdmonth=date("M",strtotime($datearray1[$i]));
					 echo '<td>';
					 echo $pcount."/".$tcount;
					 echo '</td>';
					 $pcount=$tcount=0;
				 ...
				 {
					 $tdmonth=date("M",strtotime($datearray1[$i+1]));
					 echo '<td>';
					 echo $pcount."/".$tcount;
					 echo '</td>';
					 $pcount=$tcount=0;
